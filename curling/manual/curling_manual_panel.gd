...
• 键盘不能单独发射；[b]A / D、W / S[/b]只在按住左键拖拽冰壶时微调。
• 拖得越远力量越大；窗口较短边约三分之一达到满力。
• 拖拽期间可用[b]A / D 或 ← / →[/b]微调方向，用[b]W / S 或 ↑ / ↓[/b]微调力度；按住[b]Shift[/b]会以八分之一速度慢调，不受鼠标像素精度限制。
• 整个瞄准阶段都可用[b]Q / E[/b]连续调旋，或用[b]鼠标滚轮[/b]逐格微调旋转；每格 0.08 rad/s，上限为 ±2.00 rad/s。拖拽中按鼠标右键可取消。
• 未在拖拽时可用[b]← / → 或 A / D[/b]沿赛道查看。
• 壶开始滑行后镜头自动跟踪，直到本手停稳；此阶段无需手动调整镜头。
• 瞄准限时 60 秒；超时记为空投。
• 正式比赛不显示完整预测轨迹：贯穿赛道的细直线只表示初始方向，近处的短弯曲虚线与垂直箭头只表示旋向，也不预测碰撞或落点。

[b][color=#8a4c12]新手推荐：沿中线瞄准远端 Tee，旋转保持 +0.00，不刷冰时力量拉到 77%。[/color][/b]

• 不刷冰时，[b]75%[/b] 是精确 Tee 标定；[b]77%[/b] 约滑 22.2 秒并停在 Tee 后 1.15 米，加入停稳确认后本手约 22.7 秒，仍有约 0.82 米安全余量。
• [b]78%[/b] 已接近营垒后沿，仅余约 0.24 米；[b]79%[/b] 会越过可计分范围。接近 75–77% 时，每调整 1 个百分点，停点约改变 0.6 米。
• 擦冰只能让壶滑得更远、弯得更少，不能把过重的壶减速。看起来会短时才擦，预计已经入垒就停手。
• 持续满擦约多滑 3.2 米。若计划全程擦冰，从 [b]69–70%[/b] 起手，不要仍用 77%。
• 熟悉直投后再加旋转；旋转不改变停止时间和总路径。75% 在 ±2.00 rad/s 时总路径仍为 36.59 米，但横移约 2.48 米，沿中线满旋会碰到侧边；应看轨迹弯向哪边并向反方向预留瞄准量。
...
滑行壶上方只显示一个紧凑的[b]“X.Xs”[/b]读数，表示当前预计剩余滑行时间；碰撞、擦冰状态变化或热量衰减后预测会继续更新。
...
每手开始时由 Host 判定保护对象。受保护壶带有冷色动态护膜和斜向反光，小地图显示冷色外环；右侧状态区显示保护期进度和当前数量。
