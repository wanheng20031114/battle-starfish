...
func _finish() -> void:
	...
	_check(absf(absf(cold_delta.y / CurlingConstants.PIXELS_PER_METER) - 2.49) <= 0.25, "native curl %.3fm" % (cold_delta.y / CurlingConstants.PIXELS_PER_METER))
	_check(absf(_cold_spin_at_twenty - CurlingConstants.MAX_SPIN_RADPS * 0.5) <= 0.10, "native angular half-life %.3frad/s" % _cold_spin_at_twenty)
