...
func _physics_process(delta: float) -> void:
	...
	if (_cold_stop_time >= 0.0 and _swept_stop_time >= 0.0) or _elapsed > 30.0:
		_finish()
...
func _finish() -> void:
	...
	_check(absf(cold_delta.x / CurlingConstants.PIXELS_PER_METER - 36.58) <= 0.8, "native draw distance %.3fm" % (cold_delta.x / CurlingConstants.PIXELS_PER_METER))
